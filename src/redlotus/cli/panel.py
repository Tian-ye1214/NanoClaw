...
class PanelSnapshotCache:
    ...
    def load(self, path: Path):
        try:
            ...
            journal_stat = journal.stat() if journal.exists() else stat
            signature = (
                stat.st_mtime_ns,
                stat.st_size,
...
def _collect_history(
    ...
    for path in files:
        ...
        if isinstance(summary, str):
            ...
            continue
        agent, date, topic = _file_identity(path, summary.meta)
        history.file_count += 1
        history.add_totals(summary.totals)
        history.by_agent.setdefault(agent, UsageTotals()).add_totals(summary.totals)
        for model, usage in summary.by_model.items():
            history.by_model.setdefault(model, UsageTotals()).add_totals(usage.totals)
        session = sessions.setdefault(
            (date, topic), PanelSessionSummary(date=date, topic=topic)
        )
